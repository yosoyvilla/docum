...
if ($num > 0) {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $fieldInfo = $cancha->fields($idCancha);

        while ($fieldInformation = $fieldInfo->fetch(PDO::FETCH_ASSOC)) {
            ...
            while ($rowqtn = $isRented->fetch(PDO::FETCH_ASSOC)) {
                ...
                if ($qtn === "0") {
                    ...
                    $dataRented = $cancha->getAllDataOfRented($idCancha, $dayToRent, $horario);
                    while ($dtRented = $dataRented->fetch(PDO::FETCH_ASSOC)) {
                        $field_item = array(
                            "idCancha" => $idCancha,
                            "horario" => $horario,
                            "name" => $fieldInformation['name'],
                            "idRent" => $dtRented['id'],
                            "idUserRent" => $dtRented['idUser'],
                            "status" => "Ocupada",
                        );
